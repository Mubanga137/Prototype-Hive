-- =====================================================================
-- MIGRATION: Split order messaging flow
-- Purpose: Send customer message to conversation; vendor notification to notifications table
-- Problem: Single mixed message was being sent to shared conversation visible to both parties
-- Solution: 
--   1. Customer gets system message in the conversation (already exists)
--   2. Vendor gets a separate notification row (new behavior)
-- =====================================================================
...
-- Recreate with vendor notification insertion
...
BEGIN
  ...
  BEGIN
    ...
    -- STEP 3: Insert customer-facing system message into conversation
    ...
    IF v_conversation_id IS NOT NULL THEN
      INSERT INTO public.messages (
        ...
        '🛒 Order #' || v_order_id || ' confirmed — ZMW ' || v_total_amount,
        'system_receipt',
        NOW()
      )
      ON CONFLICT DO NOTHING;
  ...
  -- STEP 4: Insert vendor notification (separate from conversation)
